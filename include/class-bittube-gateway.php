...
class BitTube_Gateway extends WC_Payment_Gateway
{
    ...
    public static function do_update_event()
    {
        ...
        $http = new WP_Http();
        $response = $http->request($api_link);
        $price = json_decode($response['body'], true);
    ...
    protected static function get_payment_details($order_id)
    {
        ...
        if (count($details)) {
            ...
            $amount_formatted = self::format_bittube($amount_due);
            $qrcode_uri = 'bittube:'.$address.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_bittube(($amount_total)/ 1e5),
                'amount_paid_formatted' => self::format_bittube($amount_paid),
                'amount_due_formatted' => self::format_bittube($amount_due),
                'status' => $status,
